...
+ (instancetype)actionWithTitle:(NSString *)title image:(UIImage *)image model:(id)model handler:(void (^)(YCMenuAction *))handler {
    YCMenuAction *action = [[YCMenuAction alloc] initWithTitle:title image:image model:model handler:handler];
    return action;
}
+ (instancetype)actionWithTitle:(NSString *)title image:(UIImage *)image handler:(void (^)(YCMenuAction *action))handler{
    YCMenuAction *action = [[YCMenuAction alloc] initWithTitle:title image:image model:nil handler:handler];
    return action;
}

- (instancetype)initWithTitle:(NSString *)title image:(UIImage *)image model:(id)model handler:(void (^)(YCMenuAction *))handler{
    if (self = [super init]) {
        _title = title;
        _image = image;
        _model = model;
