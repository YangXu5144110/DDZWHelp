...
-(void)presentSemiView:(UIView*)view
    ...
    if (![target.subviews containsObject:view]) {
        ...
        if(![[self ym_optionOrDefaultForKey:KNSemiModalOptionKeys.disableCancel] boolValue]) {
            ...
            [dismissButton addTarget:self action:@selector(dissBtn) forControlEvents:UIControlEventTouchUpInside];
...
- (void)dissBtn{
    [[NSNotificationCenter defaultCenter] postNotificationName:kSemiModalDidBackNotification
                                                        object:self];
